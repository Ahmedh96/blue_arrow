        ...
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                <div class="relative overflow-x-auto shadow-md table-responsive sm:rounded-lg">
                    <form class="forms-sample" method="POST" action="{{ route('notes.update' , $note->id) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="px-4 py-5 space-y-6 bg-white sm:p-6">
                                <div>
                                    ...
                                    <select name="type" id="type" class="flex-1 block w-full border-gray-300 rounded-none focus:ring-indigo-500 focus:border-indigo-500 rounded-r-md sm:text-sm">
                                        <option value="1" {{isset($note->type) && $note->type == 1 ? 'selected' : ''}}>urgent</option>
                                        <option value="2" {{isset($note->type) && $note->type == 2 ? 'selected' : ''}}>normal</option>
                                        <option value="3" {{isset($note->type) && $note->type == 3 ? 'selected' : ''}}>on date</option>
                                    </select>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div class="mt-1">
                                        <textarea id="content" name="content" rows="3" class="block w-full mt-1 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="Type Your Content">{{ $note->content }}</textarea>
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div class="my-2 col-md-12">
                                        <img id="preview-image-before-upload" src="https://www.riobeauty.co.uk/images/product_image_not_found.gif"
                                            ...
                                            <img src="{{ url($note->image_path) }}" style=" margin-top: 10px; width: 255px; height: 378px;" alt="">
